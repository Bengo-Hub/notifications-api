...
type UserRoleAssignment struct {
	...
	notification_role_user_assignments *uuid.UUID
...
// was not loaded in eager-loading, or loaded but was not found.
func (e UserRoleAssignmentEdges) RoleOrErr() (*NotificationRole, error) {
	if e.Role != nil {
		return e.Role, nil
...
func (*UserRoleAssignment) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case userroleassignment.ForeignKeys[0]: // notification_role_user_assignments
	...
	for i := range columns {
		...
		case userroleassignment.ForeignKeys[0]:
			if value, ok := values[i].(*sql.NullScanner); !ok {
				return fmt.Errorf("unexpected type %T for field notification_role_user_assignments", values[i])
			} else if value.Valid {
				_m.notification_role_user_assignments = new(uuid.UUID)
				*_m.notification_role_user_assignments = *value.S.(*uuid.UUID)
			}
		default:
...
func (_m *UserRoleAssignment) QueryRole() *NotificationRoleQuery {
